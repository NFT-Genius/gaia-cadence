...
transaction(ownerAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64, signatureExpiration: UInt64, signature: String) {
    ...
    let nftCollection: &CryptoPiggo.Collection{NonFungibleToken.Receiver}
    ...
    prepare(dapper: AuthAccount, acct: AuthAccount) {
        ...
        if acct.borrow<&CryptoPiggo.Collection{NonFungibleToken.Receiver}>(from: CryptoPiggo.CollectionStoragePath) == nil {
            let collection <- CryptoPiggo.createEmptyCollection() as! @CryptoPiggo.Collection
            acct.save(<-collection, to: CryptoPiggo.CollectionStoragePath)
            acct.link<&CryptoPiggo.Collection{CryptoPiggo.CryptoPiggoCollectionPublic, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(CryptoPiggo.CollectionPublicPath, target: CryptoPiggo.CollectionStoragePath)
        }

        // borrow receiver's collection
        self.nftCollection = acct.borrow<&CryptoPiggo.Collection{NonFungibleToken.Receiver}>(
            from: CryptoPiggo.CollectionStoragePath
