...
transaction(listingResourceID: UInt64, ownerAddress: Address, expectedPrice: UFix64, signatureExpiration: UInt64, signature: String) {
    ...
    prepare(dapper: AuthAccount, acct: AuthAccount) {
        ...
        if acct.borrow<&MetaPanda.Collection{NonFungibleToken.Receiver}>(from: MetaPanda.CollectionStoragePath) == nil {
            let collection <- MetaPanda.createEmptyCollection()
            acct.save(<-collection, to: MetaPanda.CollectionStoragePath)
            acct.link<&MetaPanda.Collection{NonFungibleToken.CollectionPublic,MetadataViews.ResolverCollection,AnchainUtils.ResolverCollection}>(
        ...
        self.nftCollection = acct.borrow<&MetaPanda.Collection{NonFungibleToken.Receiver}>(
