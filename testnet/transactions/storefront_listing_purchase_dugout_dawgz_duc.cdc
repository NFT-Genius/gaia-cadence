...
transaction(listingResourceID: UInt64, ownerAddress: Address, expectedPrice: UFix64, signatureExpiration: UInt64, signature: String) {
    ...
    let nftCollection: &DugoutDawgzNFT.Collection{NonFungibleToken.Receiver}
    ...
    prepare(dapper: AuthAccount, acct: AuthAccount) {
        ...
        self.nftCollection = acct.borrow<&DugoutDawgzNFT.Collection{NonFungibleToken.Receiver}>(
            from: DugoutDawgzNFT.CollectionStoragePath
