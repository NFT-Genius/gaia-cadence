...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, cutAddresses: [Address], cutPercentages: [UFix64], signatureExpiration: UInt64, signature: String) {
    ...
    prepare(acct: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/DugoutDawgzNFTCollectionProviderForNFTStorefront
        if !acct.getCapability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            acct.link<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: DugoutDawgzNFT.CollectionStoragePath)
        }
        ...
        self.oldListings = []
        let nftType = Type<@DugoutDawgzNFT.NFT>()
    ...
    execute {
        ...
        self.storefront.createListing(
            ...
            nftType: Type<@DugoutDawgzNFT.NFT>(),
            nftID: saleItemID,
