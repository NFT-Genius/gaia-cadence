...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, cutAddresses: [Address], cutPercentages: [UFix64], signatureExpiration: UInt64, signature: String) {
    ...
    prepare(acct: AuthAccount) {
        ...
        let collectionProviderPrivatePath = /private/MFLPlayerCollectionProviderForNFTStorefront

        if !acct.getCapability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(collectionProviderPrivatePath)!.check() {
            acct.link<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(collectionProviderPrivatePath, target: MFLPlayer.CollectionStoragePath)
        }
        ...
        self.oldListings = []
        let nftType = Type<@MFLPlayer.NFT>()
