...
transaction(ownerAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64, signatureExpiration: UInt64, signature: String) {
    ...
    let nftCollection: &MFLPlayer.Collection{NonFungibleToken.Receiver}
    ...
    prepare(dapper: AuthAccount, acct: AuthAccount) {
        ...
        if acct.borrow<&MFLPlayer.Collection{NonFungibleToken.Receiver}>(from: MFLPlayer.CollectionStoragePath) == nil {
        ...
        self.nftCollection = acct.borrow<&MFLPlayer.Collection{NonFungibleToken.Receiver}>(
