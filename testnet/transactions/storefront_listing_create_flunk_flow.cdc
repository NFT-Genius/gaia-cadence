...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, cutAddresses: [Address], cutPercentages: [UFix64], signatureExpiration: UInt64, signature: String) {
    let flunksProvider: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(acct: AuthAccount) {
        ...
        let flunksCollectionProviderPrivatePath = /private/FlunksCollectionProviderForNFTStorefront

        if !acct.getCapability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(flunksCollectionProviderPrivatePath)!.check() {
            acct.link<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(flunksCollectionProviderPrivatePath, target: Flunks.CollectionStoragePath)
        }

        self.flunksProvider = acct.getCapability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(flunksCollectionProviderPrivatePath)
        assert(self.flunksProvider.borrow() != nil, message: "Missing or mis-typed collection provider")
        ...
        while idx < cutAddresses.length {
            ...
            let receiver = getAccount(addr).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)
            assert(receiver.borrow() != nil, message: "Missing or mis-typed FlowToken receiver: ".concat(addr.toString()))
        ...
        if (amountRemaining > 0.0) {
            ...
            assert(receiver.borrow() != nil, message: "Missing or mis-typed seller FlowToken receiver")
            self.saleCuts.append(NFTStorefront.SaleCut(
                 receiver: receiver,
    ...
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.flunksProvider,
