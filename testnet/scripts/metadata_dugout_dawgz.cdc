...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(DugoutDawgzNFT.CollectionPublicPath)
        .borrow<&{DugoutDawgzNFT.CollectionPublic}>()
        ?? panic("Could not borrow a reference to the collection")

    let nft = collection.borrowDugoutDawgzNFT(id: listingDetails.nftID)
