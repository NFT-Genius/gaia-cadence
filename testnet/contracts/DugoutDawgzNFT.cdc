...
pub contract DugoutDawgzNFT: NonFungibleToken {
  ...
  pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
    ...
    // id is unique among all DugoutDawgzNFT NFTs on Flow, ordered sequentially from 0
    pub let id: UInt64
    ...
    init(setID: UInt64, templateID: UInt64, creator: Address) {
      self.id = DugoutDawgzNFT.totalSupply
      DugoutDawgzNFT.totalSupply = DugoutDawgzNFT.totalSupply + 1
      self.setID = setID
  ...
  // Collection
  //
  // Collections provide a way for collectors to store DugoutDawgzNFT NFTs in their
  // Flow account.

  // Exposing this interface allows external parties to inspect a Flow
  // account's DugoutDawgzNFT Collection and deposit NFTs
  pub resource interface CollectionPublic {
    pub fun deposit(token: @NonFungibleToken.NFT)
    pub fun getIDs(): [UInt64]
    pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
    pub fun borrowDugoutDawgzNFT(id: UInt64): &NFT
  }
  ...
  access(contract) fun createSet(metadata: SetMetadata): UInt64 {
    let setID = DugoutDawgzNFT.totalSets

    let newSet <- create Set(
      id: setID,
      metadata: metadata
    )
    DugoutDawgzNFT.sets[setID] <-! newSet
    DugoutDawgzNFT.totalSets = DugoutDawgzNFT.totalSets + 1
    return setID
  }
  ...
  init() {
    ...
    // Collection Paths
    self.CollectionStoragePath = /storage/DugoutDawgzNFTCollection
    self.CollectionPublicPath = /public/DugoutDawgzNFTCollection
    self.CollectionPrivatePath = /private/DugoutDawgzNFTCollection

    // Admin Storage Path. Save the singleton Admin resource to contract
    // storage.
    self.AdminStoragePath = /storage/DugoutDawgzNFTAdmin
