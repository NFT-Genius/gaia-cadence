...
import DimensionX from 0xDimensionX
import NFTStorefront from 0xNFTStorefront
import MetadataViews from 0xMetadataViews

transaction(ownerAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64, signatureExpiration: UInt64, signature: String, displayName: String, description: String, imageURL: String) {
    ...
    prepare(dapper: AuthAccount, acct: AuthAccount) {
        ...
        if acct.borrow<&DimensionX.Collection{NonFungibleToken.Receiver}>(from: DimensionX.CollectionStoragePath) == nil {
            let collection <- DimensionX.createEmptyCollection()
            acct.save(<-collection, to: DimensionX.CollectionStoragePath)
            acct.link<&DimensionX.Collection{NonFungibleToken.CollectionPublic, DimensionX.CollectionPublic, MetadataViews.ResolverCollection}>
                (DimensionX.CollectionPublicPath, target: DimensionX.CollectionStoragePath)
        }

        // borrow receiver's collection
        self.nftCollection = acct.borrow<&DimensionX.Collection{NonFungibleToken.Receiver}>(
            from: DimensionX.CollectionStoragePath
